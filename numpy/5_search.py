...
def search_and_replace():
  ...
  #           , 조건에 해당되지 않을 경우 치환할 값)
  # => 조건에 따라 변경된 값으로 구성된 배열을 반환함
...
def max_min_index():
  ...
  print(f'열 기준 최대 인덱스 : {np.argmax(arr,axis=0)}')
  print(f'행 기준 최대 인덱스 : {np.argmax(arr,axis=1)}')
# max_min_index()
