...
def normal_dist():
  '''
    정규 분포 (Normal Distribution) - 종 모양
    : 자연 현상(키, 몸무게, 시험 점수 등)에서 가장 흔하게 나타나는 분포
    : 평균을 중심으로 데이터가 모이고, 양 끝으로 갈 수록 희박해지는 모양의 그래프를 띔

    표준 편차 (Standard Deviation)
    : 데이터가 평균으로부터 얼마나 떨어져 있는지 나타내는 값
    : 표준 편차가 적을 수록 데이터가 평균 주변에 좁게 모여 있음

    np.random.normal(loc=평균, scale=표준편차, size= 개수)
  '''
  # 평균 173, 표준편차 5, 데이터 10개 생성

  standard = np.random.normal(loc=173, scale=5, size=10)
  print(standard)

  # 생성된 데이터의 평균
  print(f' - 평균 : {standard.mean():.2f}')
  # 생성된 데이터의 표준편차
  print(f' - 표준편차 : {standard.std():.2f}')
