...
# => 최빈값의 경우 여러개의 데이터가 결과로 표시되어, 값 자체를 사용할 때 첫번째 데이터를 선택!
...
#   : 상한선(Capping)을 설정하여 이상치를 찾아 대체하기

# * 95% 분위수를 초과하는 값을 이상치로 판단하여
#   설정된 상한선의 값으로 대체
...
#   사용하여 데이터의 숨겨진 패턴이나 관계를 파악하는 

# * 그룹별 복합 통계 *
#   agg : 그룹화된 데이터에 대해 여러개의 집계 함수를 한 번에 적용하여 요약 통계를 생성하는 함수
# group by 결과에 대해 sum, mean, max, std 등 집계함수를 적용
...
# * 피벗 테이블
#   pivot_table(df, index=행분류기준컬럼, coulmns=열분류기준컬럼, values=집계대상값, aggfunc= 적용할 집계함수)
